...
export class AdminFeedbackListComponent implements OnInit {
  ...
  async getFeedbacks(): Promise<void> {
    ...
    this.rideService.getFeedback().subscribe(
      (response: any) => {
        console.log("This is response", this.loading, response);
  ...
  async updateRange(): Promise<void> {
    if (this.range == 0) {
      ...
      this.displayedFeedbacks = this.feedbacks?.filter(
        (feedback) =>
          feedback.dateTime &&
          feedback.dateTime >= today &&
          feedback.dateTime < tomorrow
      );
    }
  ...
  async updateDriver(): Promise<void> {
    ...
    if (this.driverFilter != "") {
      ...
      this.displayedFeedbacks = this.feedbacks?.filter(
        (feedback) => feedback.driver && feedback.driver.match(regex)
      );
    } else {
      this.updateFilters();
    }
  ...
  async updateSubmitter(): Promise<void> {
    if (this.submitterFilter != "") {
      ...
      this.displayedFeedbacks = this.feedbacks?.filter((feedback) =>
        feedback.rider.match(regex)
      );
    } else {
      this.updateFilters();
    }
